<?php
class ProdRoadmapChangedRegion implements RoadmapChangesInterface
{
    public static function rawQuery($date, $end_date=false)
    {
        $extras = self::whereEndDate($end_date);

        return 'SELECT issues.issue_id, issues.geo_all AS geo_all, \'roadmap\' AS roadmap,'.
            ' issues.epic_name, issues.first_component,'.
            ' changed.region AS changes, changed.first_validation, changed.last_validation'.

            ' FROM roadmap_product_issues issues'.

            ' INNER JOIN roadmap_product_issues_changed_region AS changed ON issues.issue_id=changed.issue_id'.

            ' WHERE issues.issue_id IN ('
                    .'SELECT ta.issue_id  '.
                    ' FROM roadmap_product_issues_changed_region AS ta '.

                    ' WHERE ta.issue_id IN ('.
                        ' SELECT issue_id '.
                        ' FROM roadmap_product_issues_changed_region'.
