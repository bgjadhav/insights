<?php
class ProdRoadmapChangedTargetClosedBeta implements RoadmapChangesInterface
{
	public static function rawQuery($date, $end_date=false)
	{
		$extras = self::whereEndDate($end_date);

		return 'SELECT issues.issue_id, issues.target_closed_beta AS closed_beta, \'roadmap\' AS roadmap,'.
			' issues.epic_name, issues.first_component,'.
			' changed.closed_beta_release AS changes, changed.first_validation, changed.last_validation'.

			' FROM roadmap_product_issues issues'.

			' INNER JOIN roadmap_product_issues_changed_closed_beta AS changed ON issues.issue_id=changed.issue_id'.

			' WHERE issues.issue_id IN ('
					.'SELECT ta.issue_id  '.
					' FROM roadmap_product_issues_changed_closed_beta AS ta '.

					' WHERE ta.issue_id IN ('.
						' SELECT issue_id '.
						' FROM roadmap_product_issues_changed_closed_beta'.
