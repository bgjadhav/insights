...
class DownloadExchangeVideoMobileDisplayCSV implements DownloadInterface
{
	public static function output(&$out, $config)
	{
		...
		$q->execute();
		while ($row = $q->fetch()) {
			if ($config['sov']['sov_Impressions']['status'] || $config['sov']['sov_MediaCost']['status']){
				$row = ExchangeVideoMobileDisplay::formatSov($row, $config['sov'], $config['col']);
				$row = ExchangeVideoMobileDisplay::baseOrder($row, $config['base']);
			}
			$clear_row = [];
